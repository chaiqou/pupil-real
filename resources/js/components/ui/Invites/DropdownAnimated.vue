<template>
    <div @click="showHideDropdown()" href="#" class="block w-full flex-shrink-0">
     ...
     <Transition name="fade">
         <ul  @mouseleave="showHideDropdown()" v-if="isDropdownVisible" class="absolute top-1 z-10 mt-2 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-0">
             <li class="select-none p-2.5 cursor-pointer py-1.5 hover:bg-gray-200 text-sm text-gray-900" id="listbox-option-0" role="option">
                 <button @click="handleDeleteInviteRequest" class="flex flex-col">
                     <div class="flex justify-between ">
                         <h1 v-for="item in items" :key="item" class="font-normal">{{item}}</h1>
...
export default {
    ...
    methods: {
        ...
        handleDeleteInviteRequest() {
            axios.delete(`/api/${this.role}/invite/${this.inviteId}`)
                .then(res => {
