<template>
       <div @scroll="onScroll" :class="this.isInvitesLoaded && this.invites ? 'overflow-hidden max-h-[19rem] overflow-y-scroll shadow ring-1 ring-black ring-opacity-5 md:rounded-lg' : 'overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg'">
           <table class="min-w-full divide-y divide-gray-300">
               ...
               <tr>
                   ...
                   <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Send date</th>
                   <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Update date</th>
               </tr>
               </thead>
               ...
               <tr v-if="this.isInvitesLoaded  && this.invites.length" v-for="invite in invites" :key="invite.id">
                   <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{invite.email}}</td>
                   <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"> {{invite.state}} </td>
                   <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"> {{invite.created_at}} </td>
                   <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"> {{invite.updated_at}} </td>
               </tr>
