<template>
            ...
            <li class="relative md:flex md:flex-1">
                <div class="group flex w-full items-center">
                            <span class="flex items-center px-6 py-4 text-sm font-medium">
                                ...
                                <span class="ml-4 text-sm font-medium text-indigo-600">{{$t('message.setup_card')}}</span>
                            </span>
                </div>
            </li>
    ...
    <div>
        ...
        <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">{{$t('message.setup_your_card')}}</h2>
    </div>
    <div class="w-full">
            <div class="flex w-full justify-between">
                <ButtonForAxios :globalStoreSolution="true" @click="saveCard()" classOngoing="inline-flex opacity-30 justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                classDefault="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">{{$t('message.save_card_for_faster_checkout')}}</ButtonForAxios>
                <ButtonForAxios :globalStoreSolution="false" :status="axiosStatusDontSave" @click="dontSaveCard()" classOngoing="inline-flex opacity-30 justify-center rounded-md border border-[1px] bg-white py-2 px-4 text-sm font-medium shadow-sm hover:bg-gray-100 focus:outline-none"
                                classDefault="inline-flex justify-center rounded-md border border-[1px] bg-white py-2 px-4 text-sm font-medium shadow-sm hover:bg-gray-100 focus:outline-none">{{$t('message.dont_save_card')}}</ButtonForAxios>
            </div>
    </div>
</template>
<script>
