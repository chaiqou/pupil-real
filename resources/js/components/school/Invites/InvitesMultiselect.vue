...
export default {
        ...
        disabledCalculator() {
            if (this.isSent) {
                ...
                const checkIfExists = this.mainEmailsArray.find(
                    (element) =>
                        element.existsInInvites === true ||
                        element.existsInUsers === true
                );
    ...
    methods: {
        addTag(event) {
            ...
            if (event.code === "Comma" || event.code === "Enter") {
                ...
                if (emailTag.length > 0) {
                    ...
                    this.mainEmailsArray = this.emails.reduce(
                        (element, current) => {
                            const includesInviteEmail =
                                this.existedInviteEmails.includes(current);
                            const includesUserEmail =
                                this.existedUserEmails.includes(current);
                            const newValue = {
                                email: current,
        ...
        pasteTags(event) {
            setTimeout(() => {
                this.emailsPaste = this.inputValue
                    .replaceAll(",", "")
                    .split(" ");
        ...
        handleUpdateInvitesTableRequest() {
            axios
                .get(
                    `/api/school/${this.invite_from}/invites?page=${this.currentPage}`
                )
                .then((res) => {
                    this.currentPage++;
