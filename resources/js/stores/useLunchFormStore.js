...
export const useLunchFormStore = defineStore("lunch", {
  ...
  actions: {
    ...
    formatDateForHumans(name, date) {
      ...
      switch (name) {
        ...
        case "holds":
          ...
          break;
        case "add_extras":
          this.add_marked_extras = [...this.add_marked_extras, ...formatedDate];
          break;
        case "remove_holds":
    ...
    removeDaysFromMarkedDays(state) {
      ...
      while (startDate <= endDate) {
        removed_range_days.push(format(new Date(startDate), "yyyy-MM-dd"));
        startDate.setDate(startDate.getDate() + 1);
      }
      ...
      marked_days.map((day) => {
        if (
          ...
          this.each_active_range_day.includes(format(day, "yyyy-MM-dd"))
        ) {
