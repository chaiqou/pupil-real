      ...
      <div class="relative flex w-full max-w-xs flex-1 flex-col bg-white">
        ...
        <div class="h-0 flex-1 overflow-y-auto pt-5 pb-4">
          <div class="flex flex-shrink-0 items-center px-4">
            <img
              class="h-8 w-auto"
              src="https://pupilpay.hu/resc/img/pupilpay-black-color.svg"
              alt="Your Company"
            />
          </div>
        ...
        <div class="flex flex-shrink-0 border-t border-gray-200 p-4">
          <a href="#" class="group block flex-shrink-0">
            <OnClickOutside
              v-if="isMobileSwitchAccountVisible"
              @trigger="showHideMobileSwitchAccount"
            >
              <ul
                class="absolute bottom-4 z-10 mt-2 w-72 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                tabindex="-1"
                role="listbox"
                    ...
                    <a :href="item.id" class="flex flex-col">
                      <div class="flex justify-between">
                        ...
                        <span class="text-white">
                          <svg
                            :class="
                              item.id === student.id ? 'h-5 w-5' : 'hidden'
                            "
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 20 20"
                            fill="currentColor"
  ...
  methods: {
    ...
    findCurrent() {
      ...
      const navigation = this.navigation.find(
        (col) =>
          col.name.toLowerCase().replaceAll(" ", ".").replaceAll("-", ".").replaceAll("_", ".") ===
          cleanedCurrent
            .toLowerCase()
            .replaceAll(" ", ".")
            .replaceAll("-", ".")
