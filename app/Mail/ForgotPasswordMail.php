...
class ForgotPasswordMail extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
        if ($this->language === 'hu') {
            ...
            return $this->view('mail/hu/email-forgot', ['action_link' => $action_link, 'body' => 'Check your password reset link HU', 'name' => $this->name, 'browser' => $this->browser, 'device' => $this->device])->subject('Jelszó visszaállítása a PupilPay-hez');
        }
        $action_link = route('password.reset', ['token' => $this->token]);
